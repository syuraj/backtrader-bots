...
class OptionsClient:
    ...
    def get_options_chain(
        self, symbol: str, expiration_date: Optional[date] = None
    ) -> OptionsChain:
        """
        ...
        try:
            logger.info(
                "Fetching options chain", symbol=symbol, expiration=expiration_date
            )
            ...
            options_chain = OptionsChain(
                ...
                calls=(
                    contracts
                    if contracts and contracts[0].option_type == OptionType.CALL
                    else []
                ),
                puts=(
                    []
            ...
            logger.error(
                "Failed to retrieve options chain", symbol=symbol, error=str(e)
            )
    ...
    def get_historical_options_data(
        ...
        try:
            ...
            logger.info(
                "Historical options data retrieved",
                symbol=symbol,
            ...
            logger.error(
                "Failed to retrieve historical options data",
                symbol=symbol,
                error=str(e),
            )
    ...
    def _parse_option_contract(
        self, contract_data: Any, underlying_symbol: str
    ) -> OptionContract:
        ...
        try:
            ...
            date_str = (
                contract_symbol.replace(underlying_symbol, "")
                .split("C")[0]
                .split("P")[0]
            )
            ...
            contract = OptionContract(
                ...
                bid_price=(
                    Decimal(str(contract_data.bid_price))
                    if hasattr(contract_data, "bid_price")
                    else None
                ),
                ask_price=(
                    Decimal(str(contract_data.ask_price))
                    if hasattr(contract_data, "ask_price")
                    else None
                ),
                last_price=(
                    Decimal(str(contract_data.last_price))
                    if hasattr(contract_data, "last_price")
                    else None
                ),
                ...
                delta=(
                    Decimal(str(contract_data.delta))
                    if hasattr(contract_data, "delta")
                    else None
                ),
                gamma=(
                    Decimal(str(contract_data.gamma))
                    if hasattr(contract_data, "gamma")
                    else None
                ),
                theta=(
                    Decimal(str(contract_data.theta))
                    if hasattr(contract_data, "theta")
                    else None
                ),
                vega=(
                    Decimal(str(contract_data.vega))
                    if hasattr(contract_data, "vega")
                    else None
                ),
                rho=(
                    Decimal(str(contract_data.rho))
                    if hasattr(contract_data, "rho")
                    else None
                ),
                implied_volatility=(
                    Decimal(str(contract_data.implied_volatility))
                    if hasattr(contract_data, "implied_volatility")
                    else None
                ),
