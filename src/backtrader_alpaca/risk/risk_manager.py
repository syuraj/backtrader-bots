...
class PositionLimits(BaseModel):
    ...
    max_position_size: Optional[int] = Field(None, gt=0, description="Maximum position size in shares")
...
class DrawdownLimits(BaseModel):
    ...
    max_daily_loss: Optional[Decimal] = Field(None, gt=0, description="Maximum daily loss in USD")
...
class RiskMetrics(BaseModel):
    ...
    current_drawdown_pct: Decimal = Field(default=Decimal('0'), description="Current drawdown percentage")
    
    # Position metrics
    largest_position_value: Decimal = Field(default=Decimal('0'), description="Largest single position value")
    largest_position_pct: Decimal = Field(default=Decimal('0'), description="Largest position as % of portfolio")
    ...
    def update_drawdown(self):
        ...
        if self.portfolio_value > self.peak_value:
            ...
            self.current_drawdown_pct = self.current_drawdown / self.peak_value if self.peak_value > 0 else Decimal('0')
...
class RiskManager:
    ...
    def update_metrics(self, 
        ...
        daily_pnl_pct = daily_pnl / self.daily_start_value if self.daily_start_value > 0 else Decimal('0')
        ...
        for symbol, position in positions.items():
            ...
            if position_value > largest_position_value:
                ...
                largest_position_pct = position_value / portfolio_value if portfolio_value > 0 else Decimal('0')
    ...
    def _check_risk_limits(self):
        ...
        if self.drawdown_limits.max_daily_loss_pct:
            if abs(self.risk_metrics.daily_pnl_pct) > self.drawdown_limits.max_daily_loss_pct:
                self._create_alert(
                    RiskLevel.CRITICAL,
        ...
        if self.drawdown_limits.max_drawdown_pct:
            if self.risk_metrics.current_drawdown_pct > self.drawdown_limits.max_drawdown_pct:
                self._create_alert(
                    RiskLevel.CRITICAL,
        ...
        if self.position_limits.max_portfolio_concentration:
            if self.risk_metrics.largest_position_pct > self.position_limits.max_portfolio_concentration:
                self._create_alert(
                    RiskLevel.MEDIUM,
    ...
    def get_risk_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alerts': {
                ...
                'critical': len([a for a in self.alerts if a.level == RiskLevel.CRITICAL]),
